...
{
    initialize_fake_status();
    // Forced-on by default: auto-enable unless stages fail gracefully.
    mutex_lock(&selinux_hide_mutex);
    if (ksu_selinux_hide_enabled && !ksu_selinux_hide_running && ksu_selinux_hide_enable() == 0)
        ksu_selinux_hide_running = true;
    mutex_unlock(&selinux_hide_mutex);
...
{
    static_key_disable(&fake_status_initialize_key.key);
    // Retry forced-enable if second stage ran before the stock policy backup was ready.
    mutex_lock(&selinux_hide_mutex);
    if (ksu_selinux_hide_enabled && !ksu_selinux_hide_running && ksu_selinux_hide_enable() == 0)
        ksu_selinux_hide_running = true;
    mutex_unlock(&selinux_hide_mutex);
