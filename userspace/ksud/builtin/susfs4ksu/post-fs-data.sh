...
# --- Built-in deployment (built-in modules have no customize.sh) ---
DEST_BIN_DIR=/data/adb/ksu/bin
[ ! -d "$DEST_BIN_DIR" ] && mkdir -p "$DEST_BIN_DIR"
[ ! -f "$DEST_BIN_DIR/ksu_susfs" ] && cp -f "${MODDIR}/bin/arm64-v8a/ksu_susfs" "$DEST_BIN_DIR/ksu_susfs"
[ ! -f "$DEST_BIN_DIR/sus_su" ] && cp -f "${MODDIR}/bin/arm64-v8a/sus_su" "$DEST_BIN_DIR/sus_su"
chmod 755 "$DEST_BIN_DIR/ksu_susfs" "$DEST_BIN_DIR/sus_su" 2>/dev/null

# --- Persistent config init (idempotent, mirrors customize.sh) ---
[ ! -d "$PERSISTENT_DIR" ] && mkdir -p "$PERSISTENT_DIR"
for i in sus_mount.txt try_umount.txt sus_path.txt sus_path_loop.txt sus_maps.txt sus_open_redirect.txt legit_mounts.txt sus_kstat_statically.json config.sh; do
    if [ ! -f "$PERSISTENT_DIR/$i" ]; then
        cat "${MODDIR}/$i" > "$PERSISTENT_DIR/$i"
    elif [ "$i" = "config.sh" ]; then
        while IFS= read -r line; do
            key=$(echo "$line" | cut -d'=' -f1)
            [ -n "$key" ] && grep -q "^${key}=" "$PERSISTENT_DIR/$i" || echo "$line" >> "$PERSISTENT_DIR/$i"
        done < "${MODDIR}/$i"
    fi
done
